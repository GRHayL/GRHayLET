...
void GRHayLHD_InitSymBound(CCTK_ARGUMENTS) {
  ...
  if (sizeof(CCTK_REAL) < 8)
    CCTK_VERROR("Error: GRHayLHD assumes that CCTK_REAL is a double precision "
                "number. Setting otherwise will likely cause havoc with the "
                "conserv_to_prims solver.");

  if ((cctk_nghostzones[0] < 3 || cctk_nghostzones[1] < 3 ||
       cctk_nghostzones[2] < 3))
    CCTK_VERROR("ERROR: The version of PPM in this thorn requires 3 "
                "ghostzones. You only have (%d,%d,%d) ghostzones!",
                cctk_nghostzones[0], cctk_nghostzones[1], cctk_nghostzones[2]);

  if (cctk_iteration == 0) {
    CCTK_VINFO("Setting Symmetry = %s... at iteration = %d", Symmetry,
               cctk_iteration);
    ...
    if (CCTK_EQUALS(Symmetry, "none")) {
      ...
      // Set default to no symmetry, which is correct for scalars and most
      // vectors:
      sym[0] = 1;
      sym[1] = 1;
      sym[2] = 1;
      ...
      CCTK_ERROR("GRHayLHD_initsymbound: Should not be here; picked an "
                 "impossible symmetry.");
    }
  }
}
