...
static inline int GRHayLMHD_local_avg(
    ...
    const CCTK_REAL *restrict Stildez, const CCTK_REAL *restrict Ye_star,
...
void GRHayLHD_tabulated_conservs_to_prims(CCTK_ARGUMENTS) {
  DECLARE_CCTK_ARGUMENTS_GRHayLHD_tabulated_conservs_to_prims;
    ...
    for(int j=0; j<jmax; j++) {
      for(int i=0; i<imax; i++) {
        ...
        if (isnan(cons.rho * cons.tau * cons.SD[0] * cons.SD[1] * cons.SD[2] *
                  cons.Y_e * prims.BU[0] * prims.BU[1] *
                  prims.BU[2])) {
        ...
        if(cons.rho>0.0) {
          ...
          if(check || isnan(prims.rho * prims.press * prims.eps * prims.vU[0] *
                             prims.vU[1] * prims.vU[2] * prims.Y_e * prims.temperature)) {
            needs_average[index] = 1;
            n_avg++;
            continue;
          }
  ...
  */
  if(CCTK_Equals(verbose, "yes")) {
    CCTK_VINFO(
        ...
        Ye_error, error_Ye_denom, Sx_error, error_Sx_denom,
