...
void IllinoisGRMHD_enforce_primitive_limits_and_compute_conservs(const cGH* cctkGH, const int index);
...
#pragma omp parallel for
      for(int k=0; k<cctk_lsh[2]; k++) {
        for(int j=0; j<cctk_lsh[1]; j++) {
          ...
          IllinoisGRMHD_enforce_primitive_limits_and_compute_conservs(cctkGH, index);
        }
      }
    }
