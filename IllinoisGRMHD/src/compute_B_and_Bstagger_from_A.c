...
#pragma omp parallel for
  for(int k=0; k<cctk_lsh[2]; k++) {
    for(int j=0; j<cctk_lsh[1]; j++) {
      for(int i=0; i<cctk_lsh[0]; i++) {
        ...
        Bx[actual_index] = 0.5 * ( Bx_stagger[index] + Bx_stagger[indexim1] )/sqrt_detgamma;
        ...
        By[actual_index] = 0.5 * ( By_stagger[index] + By_stagger[indexjm1] )/sqrt_detgamma;
        ...
        Bz[actual_index] = 0.5 * ( Bz_stagger[index] + Bz_stagger[indexkm1] )/sqrt_detgamma;
      }
    }
  }
  ...
  IllinoisGRMHD_set_symmetry_gzs_staggered( cctkGH, x, y, z,  Bx,  gridfunc_syms_Bx, 0, 0, 0);
  ...
  IllinoisGRMHD_set_symmetry_gzs_staggered( cctkGH, x, y, z,  By,  gridfunc_syms_By, 0, 0, 0);
  ...
  IllinoisGRMHD_set_symmetry_gzs_staggered( cctkGH, x, y, z,  Bz,  gridfunc_syms_Bz, 0, 0, 0);
