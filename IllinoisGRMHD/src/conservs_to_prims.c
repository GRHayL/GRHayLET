...
void IllinoisGRMHD_conserv_to_prims(CCTK_ARGUMENTS) {
  DECLARE_CCTK_ARGUMENTS_IllinoisGRMHD_conserv_to_prims;
    ...
    for(int k=0; k<kmax; k++) {
      for(int j=0; j<jmax; j++) {
        for(int i=0; i<imax; i++) {
          ...
          mhd_st_x[index] *= one_plus_pert(perturb_every_con2prim);
          mhd_st_y[index] *= one_plus_pert(perturb_every_con2prim);
          mhd_st_z[index] *= one_plus_pert(perturb_every_con2prim);

          Bx[index] *= one_plus_pert(perturb_every_con2prim);
          By[index] *= one_plus_pert(perturb_every_con2prim);
          Bz[index] *= one_plus_pert(perturb_every_con2prim);
        }
      }
    }
  }
