...
void IllinoisGRMHD_tabulated_entropy_enforce_primitive_limits_and_compute_conservs(const cGH* cctkGH, const int index, ghl_primitive_quantities *restrict prims);
 ...
 *******************************************************/
void IllinoisGRMHD_tabulated_entropy_hydro_outer_boundaries(CCTK_ARGUMENTS) {
  DECLARE_CCTK_ARGUMENTS_IllinoisGRMHD_tabulated_entropy_hydro_outer_boundaries;
...
#pragma omp parallel for
      for(int k=0; k<cctk_lsh[2]; k++) {
        for(int j=0; j<cctk_lsh[1]; j++) {
          ...
          IllinoisGRMHD_tabulated_entropy_enforce_primitive_limits_and_compute_conservs(cctkGH, index, &prims);
        }
      }
    }
