...
void IllinoisGRMHD_tabulated_entropy_calculate_flux_dir_rhs(
  ...
  for(int k=kmin; k<kmax+1; k++) {
    for(int j=jmin; j<jmax+1; j++) {
      for(int i=imin; i<imax+1; i++) {
        ...
        ghl_calculate_HLLE_fluxes_tabulated_entropy(flux_dir, ghl_eos, &ADM_metric_face, &prims_r, &prims_l, cmin[index], cmax[index], &cons_fluxes);
