...
void GRHayLMHD_tabulated_entropy_calculate_flux_dir_rhs(
    const cGH *restrict cctkGH, const int flux_dir, const CCTK_REAL **B_center,
    const CCTK_REAL *restrict B_stagger, CCTK_REAL **vel_r, CCTK_REAL **vel_l,
    CCTK_REAL *restrict cmin, CCTK_REAL *restrict cmax) {
  DECLARE_CCTK_ARGUMENTS_GRHayLMHD_tabulated_entropy_evaluate_fluxes_rhs;
  ...
  void (*calculate_characteristic_speed)(
      ...
      const ghl_metric_quantities *restrict ADM_metric_face, double *cmin,
      double *cmax);
  ...
  void (*calculate_HLLE_fluxes)(
      ...
      ghl_primitive_quantities *restrict prims_l,
      const ghl_eos_parameters *restrict eos,
      const ghl_metric_quantities *restrict ADM_metric_face, const double cmin,
      const double cmax, ghl_conservative_quantities *restrict cons_fluxes);
  ...
  case 2:
    ...
    break;
  default:
    CCTK_ERROR("Invalid flux_dir value (not 0, 1, or 2) has been passed to "
               "calculate_MHD_rhs.");
  }

  // This loop fills in all the data for reconstructed velocities. This loop is
  // larger than the others because we will need to reconstruct a second time.
  ...
  const int vimax =
      cctkGH->cctk_lsh[0] - xdir * (cctkGH->cctk_nghostzones[0] - 1);
  const int vjmax =
      cctkGH->cctk_lsh[1] - ydir * (cctkGH->cctk_nghostzones[1] - 1);
  const int vkmax =
      cctkGH->cctk_lsh[2] - zdir * (cctkGH->cctk_nghostzones[2] - 1);
...
#pragma omp parallel for
    ...
    for (int j = vjmin; j < vjmax; j++) {
      for (int i = vimin; i < vimax; i++) {
        ...
        for (int ind = 0; ind < 6; ind++) {
          ...
          const int stencil =
              CCTK_GFINDEX3D(cctkGH, i + xdir * (ind - 3), j + ydir * (ind - 3),
                             k + zdir * (ind - 3));
          v_flux_dir[ind] =
              v_flux[stencil]; // Could be smaller; doesn't use full stencil
  ...
  // This loop includes 1 ghostzone because the RHS calculation for e.g. the x
  // direction requires (i,j,k) and (i+1,j,k); if cmin/max weren't also needed
  // for A_i, we could technically have the loop only go 1 extra point in the
  // flux_dir direction
#pragma omp parallel for
  for (int k = kmin; k < kmax + 1; k++) {
    for (int j = jmin; j < jmax + 1; j++) {
      for (int i = imin; i < imax + 1; i++) {
        ...
        GRHayLMHD_interpolate_metric_to_face(cctkGH, i, j, k, flux_dir, alp,
                                             betax, betay, betaz, gxx, gxy, gxz,
                                             gyy, gyz, gzz, &ADM_metric_face);
        ...
        ghl_ppm_reconstruction_with_steepening(ghl_params, press_stencil, Gamma,
                                               ftilde, rho_stencil,
                                               &prims_r.rho, &prims_l.rho);
        ...
        prims_r.BU[B_recon[0]] = prims_l.BU[B_recon[0]] =
            B_stagger[indm1] / ADM_metric_face.sqrt_detgamma;
        ...
        int speed_limited CCTK_ATTRIBUTE_UNUSED =
            ghl_limit_v_and_compute_u0(ghl_params, &ADM_metric_face, &prims_r);
        speed_limited =
            ghl_limit_v_and_compute_u0(ghl_params, &ADM_metric_face, &prims_l);

        // We must now compute eps and T
        ghl_tabulated_enforce_bounds_rho_Ye_P(ghl_eos, &prims_r.rho,
                                              &prims_r.Y_e, &prims_r.press);
        ghl_tabulated_compute_eps_T_from_P(ghl_eos, prims_r.rho, prims_r.Y_e,
                                           prims_r.press, &prims_r.eps,
                                           &prims_r.temperature);

        ghl_tabulated_enforce_bounds_rho_Ye_P(ghl_eos, &prims_l.rho,
                                              &prims_l.Y_e, &prims_l.press);
        ghl_tabulated_compute_eps_T_from_P(ghl_eos, prims_l.rho, prims_l.Y_e,
                                           prims_l.press, &prims_l.eps,
                                           &prims_l.temperature);

        ghl_conservative_quantities cons_fluxes;
        calculate_characteristic_speed(&prims_r, &prims_l, ghl_eos,
                                       &ADM_metric_face, &cmin[index],
                                       &cmax[index]);
        calculate_HLLE_fluxes(&prims_r, &prims_l, ghl_eos, &ADM_metric_face,
                              cmin[index], cmax[index], &cons_fluxes);
  ...
  for (int k = kmin; k < kmax; k++) {
    for (int j = jmin; j < jmax; j++) {
      for (int i = imin; i < imax; i++) {
        ...
        rho_star_rhs[index] +=
            dxi * (rho_star_flux[index] - rho_star_flux[indp1]);
        ...
        ent_star_rhs[index] +=
            dxi * (ent_star_flux[index] - ent_star_flux[indp1]);
