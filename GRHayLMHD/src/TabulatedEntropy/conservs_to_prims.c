#include "GRHayLMHD.h"
#include "GRHayLMHD_local_avg.h"

void GRHayLMHD_tabulated_entropy_conserv_to_prims(CCTK_ARGUMENTS) {
  DECLARE_CCTK_ARGUMENTS_GRHayLMHD_tabulated_entropy_conserv_to_prims;
...
#pragma omp parallel for reduction(                                            \
        + : pointcount, backup0, backup1, backup2, vel_limited_ptcount,        \
            rho_star_fix_applied, failures, failures_inhoriz, n_avg,           \
    ...
    for (int j = 0; j < jmax; j++) {
      for (int i = 0; i < imax; i++) {
        ...
        if (isnan(cons.rho * cons.tau * cons.SD[0] * cons.SD[1] * cons.SD[2] *
                  cons.entropy * cons.Y_e * prims.BU[0] * prims.BU[1] *
                  prims.BU[2])) {
