        ...
        if(cctk_bbox[GRHayLMHD_xmin]) {
            for(int k = 0; k < cctk_lsh[2]; k++) {
                for(int j = 0; j < cctk_lsh[1]; j++) {
                    ...
                    A_x_tilde[ijk] = 2.0 * A_x_tilde[ip1jk] - A_x_tilde[ip2jk];
                    A_y_tilde[ijk] = 2.0 * A_y_tilde[ip1jk] - A_y_tilde[ip2jk];
                    A_z_tilde[ijk] = 2.0 * A_z_tilde[ip1jk] - A_z_tilde[ip2jk];
                    Phi_tilde[ijk] = 2.0 * Phi_tilde[ip1jk] - Phi_tilde[ip2jk];
                }
            }
        }
        ...
        if(cctk_bbox[GRHayLMHD_xmax]) {
            for(int k = 0; k < cctk_lsh[2]; k++) {
                for(int j = 0; j < cctk_lsh[1]; j++) {
                    ...
                    A_x_tilde[ijk] = 2.0 * A_x_tilde[im1jk] - A_x_tilde[im2jk];
                    A_y_tilde[ijk] = 2.0 * A_y_tilde[im1jk] - A_y_tilde[im2jk];
                    A_z_tilde[ijk] = 2.0 * A_z_tilde[im1jk] - A_z_tilde[im2jk];
                    Phi_tilde[ijk] = 2.0 * Phi_tilde[im1jk] - Phi_tilde[im2jk];
                }
            }
        }
        ...
        if(cctk_bbox[GRHayLMHD_ymin]) {
            for(int k = 0; k < cctk_lsh[2]; k++) {
                for(int i = 0; i < cctk_lsh[0]; i++) {
                    ...
                    A_x_tilde[ijk] = 2.0 * A_x_tilde[ijp1k] - A_x_tilde[ijp2k];
                    A_y_tilde[ijk] = 2.0 * A_y_tilde[ijp1k] - A_y_tilde[ijp2k];
                    A_z_tilde[ijk] = 2.0 * A_z_tilde[ijp1k] - A_z_tilde[ijp2k];
                    Phi_tilde[ijk] = 2.0 * Phi_tilde[ijp1k] - Phi_tilde[ijp2k];
                }
            }
        }
        ...
        if(cctk_bbox[GRHayLMHD_ymax]) {
            for(int k = 0; k < cctk_lsh[2]; k++) {
                for(int i = 0; i < cctk_lsh[0]; i++) {
                    ...
                    A_x_tilde[ijk] = 2.0 * A_x_tilde[ijm1k] - A_x_tilde[ijm2k];
                    A_y_tilde[ijk] = 2.0 * A_y_tilde[ijm1k] - A_y_tilde[ijm2k];
                    A_z_tilde[ijk] = 2.0 * A_z_tilde[ijm1k] - A_z_tilde[ijm2k];
                    Phi_tilde[ijk] = 2.0 * Phi_tilde[ijm1k] - Phi_tilde[ijm2k];
                }
            }
        }
        ...
        if(cctk_bbox[GRHayLMHD_zmin]) {
            for(int j = 0; j < cctk_lsh[1]; j++) {
                for(int i = 0; i < cctk_lsh[0]; i++) {
                    ...
                    A_x_tilde[ijk] = 2.0 * A_x_tilde[ijkp1] - A_x_tilde[ijkp2];
                    A_y_tilde[ijk] = 2.0 * A_y_tilde[ijkp1] - A_y_tilde[ijkp2];
                    A_z_tilde[ijk] = 2.0 * A_z_tilde[ijkp1] - A_z_tilde[ijkp2];
                    Phi_tilde[ijk] = 2.0 * Phi_tilde[ijkp1] - Phi_tilde[ijkp2];
                }
            }
        }
        ...
        if(cctk_bbox[GRHayLMHD_zmax]) {
            for(int j = 0; j < cctk_lsh[1]; j++) {
                for(int i = 0; i < cctk_lsh[0]; i++) {
                    ...
                    A_x_tilde[ijk] = 2.0 * A_x_tilde[ijkm1] - A_x_tilde[ijkm2];
                    A_y_tilde[ijk] = 2.0 * A_y_tilde[ijkm1] - A_y_tilde[ijkm2];
                    A_z_tilde[ijk] = 2.0 * A_z_tilde[ijkm1] - A_z_tilde[ijkm2];
                    Phi_tilde[ijk] = 2.0 * Phi_tilde[ijkm1] - Phi_tilde[ijkm2];
                }
            }
        }
    }
