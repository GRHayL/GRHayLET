  ...
  for (int k = 0; k < cctk_lsh[2]; k++) {
    for (int j = 0; j < cctk_lsh[1]; j++) {
      for (int i = 0; i < cctk_lsh[0]; i++) {
        ...
        /* Note that we currently do not set Abar, Y_e, temperature, entropy,
         * Avec[3], Aphi, Avec_stag[3], Aphi_stag */
        ...
        if (one_minus_invW_squared > 1.0) {
          CCTK_VINFO(
              "convert_from_GRHayLMHD_to_HydroBase WARNING: Found superluminal "
              "velocity. This should have been caught by GRHayLMHD.");
        }
