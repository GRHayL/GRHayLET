...
#define GRHAYLMHD_COMPUTE_METRIC_DERIVS(derivs_struct, dir)                                       \
    {                                                                                             \
        const int di = kronecker_delta[dir][0];                                                   \
        const int dj = kronecker_delta[dir][1];                                                   \
        const int dk = kronecker_delta[dir][2];                                                   \
        ...
        const int p2 = CCTK_GFINDEX3D(cctkGH, i + 2 * di, j + 2 * dj, k + 2 * dk);                \
                                                                                                  \
        const CCTK_REAL deriv_alp   = GRHAYLMHD_COMPUTE_DERIV(alp, m2, m1, p1, p2, inv_h[dir]);   \
        const CCTK_REAL deriv_betax = GRHAYLMHD_COMPUTE_DERIV(betax, m2, m1, p1, p2, inv_h[dir]); \
        const CCTK_REAL deriv_betay = GRHAYLMHD_COMPUTE_DERIV(betay, m2, m1, p1, p2, inv_h[dir]); \
        const CCTK_REAL deriv_betaz = GRHAYLMHD_COMPUTE_DERIV(betaz, m2, m1, p1, p2, inv_h[dir]); \
        const CCTK_REAL deriv_gxx   = GRHAYLMHD_COMPUTE_DERIV(gxx, m2, m1, p1, p2, inv_h[dir]);   \
        const CCTK_REAL deriv_gxy   = GRHAYLMHD_COMPUTE_DERIV(gxy, m2, m1, p1, p2, inv_h[dir]);   \
        const CCTK_REAL deriv_gxz   = GRHAYLMHD_COMPUTE_DERIV(gxz, m2, m1, p1, p2, inv_h[dir]);   \
        const CCTK_REAL deriv_gyy   = GRHAYLMHD_COMPUTE_DERIV(gyy, m2, m1, p1, p2, inv_h[dir]);   \
        const CCTK_REAL deriv_gyz   = GRHAYLMHD_COMPUTE_DERIV(gyz, m2, m1, p1, p2, inv_h[dir]);   \
        const CCTK_REAL deriv_gzz   = GRHAYLMHD_COMPUTE_DERIV(gzz, m2, m1, p1, p2, inv_h[dir]);   \
...
void GRHayLMHD_Compute_RHSs_Sources(CCTK_ARGUMENTS)
{
    ...
    OMPLOOP3D(imin, imax, jmin, jmax, kmin, kmax)
    {
        ...
        GRHAYLMHD_COMPUTE_METRIC_DERIVS(metric_derivs[0], 0);
        GRHAYLMHD_COMPUTE_METRIC_DERIVS(metric_derivs[1], 1);
        GRHAYLMHD_COMPUTE_METRIC_DERIVS(metric_derivs[2], 2);
