...
void GRHayLMHD_tabulated_conservs_to_prims(CCTK_ARGUMENTS) {
  DECLARE_CCTK_ARGUMENTS_GRHayLMHD_tabulated_conservs_to_prims;
      ...
      for(int i=0; i<imax; i++) {
        ...
        if (cons.rho > 0.0 &&
            isfinite(cons.rho*cons.tau*cons.SD[0]*cons.SD[1]*cons.SD[2]*
                     prims.BU[0]*prims.BU[1]*prims.BU[2]*cons.Y_e)) {
