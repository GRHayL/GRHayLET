...
void GRHayLMHD_tabulated_conserv_to_prims(CCTK_ARGUMENTS) {
  DECLARE_CCTK_ARGUMENTS_GRHayLMHD_tabulated_conserv_to_prims;
...
#pragma omp parallel for reduction(+: \
      ...
      error_rho_numer, error_tau_numer, error_Sx_numer, error_Sy_numer, error_Sz_numer, error_Ye_numer, \
      error_rho_denom, error_tau_denom, error_Sx_denom, error_Sy_denom, error_Sz_denom, error_Ye_denom) schedule(static)
  for(int k=0; k<kmax; k++) {
    for(int j=0; j<jmax; j++) {
      for(int i=0; i<imax; i++) {
        ...
        if(isnan(cons.rho*cons.tau*cons.SD[0]*cons.SD[1]*cons.SD[2]*cons.Y_e*prims.BU[0]*prims.BU[1]*prims.BU[2])) {
        ...
        if(cons.rho>0.0) {
          ...
          if(check || isnan(prims.rho*prims.press*prims.eps*prims.vU[0]*prims.vU[1]*prims.vU[2]*prims.Y_e*prims.temperature) ) {
            needs_average[index] = 1;
            n_avg++;
            continue;
          }
  ...
  if(n_avg>0) {
    ...
    while(n_avg > 0 && avg_weight < 5) {
      ...
      for(int iter=0; iter<n_avg; iter++) {
        ...
        if(avg_fail || 
           isnan(cons.rho*cons.tau*cons.SD[0]*cons.SD[1]*cons.SD[2]*cons.Y_e*prims.BU[0]*prims.BU[1]*prims.BU[2])) {
